...
function AnimatedBackground() {
  return (
    <div className="fixed inset-0 -z-10 overflow-hidden">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_20%_30%,rgba(184,212,144,0.08),transparent_50%),radial-gradient(ellipse_at_80%_70%,rgba(184,212,144,0.05),transparent_50%)]" />
      <motion.div
      ...
      <motion.div
        ...
        style={{ background: `radial-gradient(circle, ${COLOR}05, transparent)` }}
...
function LiveChatDemo() {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0d1117]/80 backdrop-blur-sm overflow-hidden shadow-2xl">
      {/* Header */}
      ...
      <div className="grid lg:grid-cols-[1.5fr_0.5fr]">
        <div className="p-5 min-h-[480px] flex flex-col">
              ...
              {messages.map((msg, idx) => (
                ...
                >
                  <div className={`max-w-[85%] space-y-1 ${msg.from === 'bob' ? 'items-end' : 'items-start'} flex flex-col`}>
                    <span className="text-[10px] font-mono text-white/40 px-1">{msg.from}</span>
                    <div
                      ...
                      style={{ color: msg.from === 'bob' ? '#eaf7d2' : '#c9d1d9' }}
                    >
                      {msg.text}
                    </div>
          ...
          <div className="mt-4 pt-4 border-t border-white/10">
            <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-black/30 p-2">
              <div className="flex-1 px-3 py-2 text-xs text-white/40 font-mono">
        ...
        {/* Encryption Pipeline */}
        <div className="border-l border-white/10 bg-black/20 p-5">
          ...
          <div className="mt-5 pt-4 border-t border-white/10">
            <div className="flex items-center justify-between text-[11px]">
              <span className="text-white/40">Current Key Size</span>
...
// Mobile Menu
function MobileMenu({ isOpen, setIsOpen }: { isOpen: boolean; setIsOpen: (v: boolean) => void }) {
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 z-50 h-full w-72 border-l border-white/10 bg-[#0d1117] p-6"
          >
            <button onClick={() => setIsOpen(false)} className="mb-8 ml-auto block text-white/60">
              <X className="h-5 w-5" />
            </button>
            <div className="flex flex-col gap-5">
              <a href="#features" className="text-white/70 hover:text-white transition">Features</a>
              <a href="#security" className="text-white/70 hover:text-white transition">Security</a>
...
export default function LandingPage() {
  ...
  return (
    <main className="min-h-screen text-white bg-[#0d1117] overflow-x-hidden">
      <AnimatedBackground />
      <MobileMenu isOpen={mobileMenuOpen} setIsOpen={setMobileMenuOpen} />

      {/* Navigation */}
      <nav className={`fixed top-0 z-40 w-full transition-all duration-300 ${
        isScrolled ? 'bg-[#0d1117]/90 backdrop-blur-xl border-b border-white/10' : 'bg-transparent'
      }`}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3 group">
            <div>
              <span className="font-bold text-lg tracking-tight">CRYPT<span style={{ color: COLOR }}>CHAT</span></span>
              <p className="text-[9px] text-white/35 -mt-0.5 tracking-wide">SECURE COMMUNICATION</p>
            </div>
          </Link>

          <div className="hidden lg:flex items-center gap-8 text-sm">
            <a href="#features" className="text-white/50 hover:text-white transition">Features</a>
            <a href="#security" className="text-white/50 hover:text-white transition">Security</a>
            <a href="#architecture" className="text-white/50 hover:text-white transition">Architecture</a>
            <a href="#metrics" className="text-white/50 hover:text-white transition">Metrics</a>
          </div>
          ...
          <button className="lg:hidden text-white/60" onClick={() => setMobileMenuOpen(true)}>
      ...
      <section className="relative pt-32 pb-20 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            >
              <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-xs font-mono mb-6" style={{ color: COLOR }}>
              ...
              <p className="mt-6 text-base text-white/45 leading-relaxed max-w-lg">
              ...
              <div className="flex flex-wrap gap-4 mt-8">
                ...
                <Link href="/dashboard">
                  <button className="px-6 py-3 rounded-xl border border-white/10 bg-white/5 text-white/70 transition hover:bg-white/10">
                    View Metrics
                  </button>
                </Link>
              </div>
      ...
      <section id="features" className="py-20 px-6 border-t border-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-white/40 mt-3">Every layer engineered for confidentiality and performance.</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map((feature, idx) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className="group p-5 rounded-xl border border-white/10 bg-white/[0.02] hover:bg-white/[0.04] transition-all hover:-translate-y-0.5"
              >
      ...
      {/* Security Section */}
      <section id="security" className="py-20 px-6 border-t border-white/5 bg-white/[0.01]">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="rounded-xl border border-white/10 bg-black/30 p-5 font-mono text-xs overflow-x-auto">
...
{`async function encryptMessage(message, recipientPubKey) {
  ...
  return { ciphertext, wrappedKey, signature };
      ...
      {/* Metrics Section */}
      <section id="metrics" className="py-16 px-6 border-t border-white/5">
        <div className="max-w-7xl mx-auto">
      ...
      <section className="py-20 px-6 text-center border-t border-white/5">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-white/45 mb-8">Start encrypting your conversations with zero-knowledge architecture.</p>
          <Link href="/login">
      ...
      <footer className="py-6 px-6 border-t border-white/5 text-center">
        <p className="text-xs text-white/25">© 2026 CRYPTCHAT — Zero-Knowledge Encrypted Messaging System</p>
      </footer>
    </main>
  );
}